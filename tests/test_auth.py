...
# conftest sets LIFF_ID = "test-liff-id", so channel_id = "test"
_MOCK_VERIFY_OK = {
    "client_id": "test",
    "sub": LINE_USER_ID,
    "expires_in": 2592000,
...
def test_verify_liff_token_success(mock_get: MagicMock) -> None:
    mock_get.return_value = MagicMock(
        status_code=200, json=lambda: _MOCK_VERIFY_OK
    )
...
@patch("app.services.line_service.httpx.get")
def test_verify_liff_token_channel_mismatch(mock_get: MagicMock) -> None:
    mock_get.return_value = MagicMock(
        status_code=200,
        json=lambda: {"client_id": "other-channel", "sub": LINE_USER_ID},
    )
    with pytest.raises(LineAuthError, match="token_channel_mismatch"):
        line_service.verify_liff_token(VALID_TOKEN)


@patch("app.services.line_service.httpx.get")
def test_verify_liff_token_missing_sub(mock_get: MagicMock) -> None:
    mock_get.return_value = MagicMock(
        status_code=200, json=lambda: {"client_id": "test"}
    )
